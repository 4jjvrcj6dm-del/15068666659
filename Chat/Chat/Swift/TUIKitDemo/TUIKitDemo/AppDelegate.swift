...
class AppDelegate: UIResponder, UIApplicationDelegate, V2TIMConversationListener, TUILoginListener, TUIThemeSelectControllerDelegate, TUILanguageSelectControllerDelegate, V2TIMAPNSListener, V2TIMSDKListener, TIMPushDelegate, TIMPushListener {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        // 先设置rootViewController，再让window可见
    ...
    /// 初始化时设置rootViewController，避免window显示时没有根控制器导致崩溃
    ...
    func tryAutoLogin() {
        ...
        if let uid = userID, let sig = userSig, !uid.isEmpty, !sig.isEmpty {
            loginSDK(uid, userSig: sig, succ: nil, fail: nil)
        }
    }
    ...
    @objc func navigateToBuiltInChatViewControllerImpl(_ userID: String?, groupID: String?) {
        ...
        let conversationControllerClass = NSClassFromString("ConversationController")
        let minimalistControllerClass = NSClassFromString("ConversationController_Minimalist")
        
        var isCorrectClass = false
        if let convClass = conversationControllerClass {
            isCorrectClass = vc.isKind(of: convClass)
        }
        if !isCorrectClass, let miniClass = minimalistControllerClass {
            isCorrectClass = vc.isKind(of: miniClass)
        }
        ...
        // The method expects (groupID, userID) order based on the selector name
        let selector = NSSelectorFromString("pushToChatViewController:userID:")
        if vc.responds(to: selector) {
            // Convert to AnyObject? to avoid potential type issues
            let groupIdParam: AnyObject? = (groupID as NSString?) ?? NSNull()
            let userIdParam: AnyObject? = (userID as NSString?) ?? NSNull()
            vc.perform(selector, with: groupIdParam, with: userIdParam)
        }
    }
